...
- @spec-library-panel.md → especificación técnica maestra del módulo concreto que quiero implementar o refactorizar.
...
Tu tarea es TRANSFORMAR @spec-library-panel.md en una lista de prompts secuenciados para implementar ese módulo en código dentro de src\features\EditorEntrenamiento\components\LibraryPanel.tsx, utilizando @editor_entrenamiento_definitivo.md como guía de coherencia visual y funcional con el resto del editor.
...
- `@spec-library-panel.md` → nombre del archivo de especificación del módulo.
- `src\features\EditorEntrenamiento\components\LibraryPanel.tsx` → ruta del archivo principal de React/TypeScript donde se va a trabajar.
...
2. Asumir que yo, antes de usar este prompt inicial, haré un “buscar y reemplazar”:
   - `@spec-library-panel.md` → por el nombre real del archivo de especificación (ej.: `@spec-editor-canvas.md`, `@spec-batch-training-modal.md`, etc.).
   - `src\features\EditorEntrenamiento\components\LibraryPanel.tsx` → por la ruta real del archivo principal (ej.: `src/features/EditorEntrenamiento/components/EditorCanvas.tsx`).
...
"**01_Modulo_Foundation**: INICIO DEL MONTAJE. Quiero que configures la estructura base del módulo en `src\features\EditorEntrenamiento\components\LibraryPanel.tsx`.\n\n**Instrucciones:**\n1. ...\n2. ...\n\n**Referencias Obligatorias:**\n- Revisa @spec-library-panel.md (Secciones relevantes que indiques).\n- Revisa @editor_entrenamiento_definitivo.md (para mantener consistencia de layout, estilos y comportamiento global)."
...
1. **Ruta de archivo obligatoria en TODOS los prompts**
   - Cada prompt debe indicar SIEMPRE que el trabajo se hará en:
     - `src\features\EditorEntrenamiento\components\LibraryPanel.tsx`
   - Puede mencionar la creación de componentes auxiliares (por ejemplo `Header`, `Sidebar`, `DayCard`, `BatchTrainingPanel`, etc.), pero dejando claro que se integran o usan desde `src\features\EditorEntrenamiento\components\LibraryPanel.tsx`.
...
2. **Referencias obligatorias en TODOS los prompts**
   - Cada prompt debe cerrar con un bloque `**Referencias Obligatorias:**` que incluya SIEMPRE:
     - `- Revisa @spec-library-panel.md (Secciones X, Y, Z que tú estimes relevantes).`
     - `- Revisa @editor_entrenamiento_definitivo.md (para mantener coherencia con el resto del editor de entrenamiento).`
   - Mantén exactamente estos nombres con la arroba delante: `@spec-library-panel.md` y `@editor_entrenamiento_definitivo.md`.

3. **Orden y numeración (plan paso a paso)**
   - Los prompts deben estar ORDENADOS como un plan de implementación incremental para ese módulo. A partir de la estructura de @spec-library-panel.md, descompón el trabajo en fases lógicas, por ejemplo:
...
4. **Estructura interna de CADA prompt**
   - Todos los prompts deben seguir SIEMPRE esta estructura:
     ...
     - Párrafo inicial: Explica qué se va a construir o refactorizar y deja claro que el foco es `src\features\EditorEntrenamiento\components\LibraryPanel.tsx`.
...
5. **Cobertura completa del módulo**
   - A partir de @spec-library-panel.md:
...
6. **Estilo y enfoque**
   - Claros, accionables, sin ambigüedades y orientados a implementación real en React + TypeScript + Tailwind dentro de `src\features\EditorEntrenamiento\components\LibraryPanel.tsx`.
   - Si @spec-library-panel.md menciona integración con otros módulos (por ejemplo navegación global, top bar, batch training, etc.), incluye en los prompts referencias explícitas a esas interacciones y apunta a @editor_entrenamiento_definitivo.md para asegurar consistencia.

Recuerda: la salida final debe ser SOLO el JSON con la clave `"prompts"` y todos los prompts ya ordenados, numerados, mejorados y listos para usar con Gemini CLI, utilizando los literales `@spec-library-panel.md` y `src\features\EditorEntrenamiento\components\LibraryPanel.tsx` tal cual aparezcan en este prompt inicial.

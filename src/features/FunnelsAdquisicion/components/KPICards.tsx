...
import { MetricCardData, MetricCards } from '../../../components/componentsreutilizables';
import { AcquisitionKPI } from '../types';
...
const periodLabel: Record<AcquisitionKPI['period'], string> = {
  '7d': 'Últimos 7 días',
  '30d': 'Últimos 30 días',
  '90d': 'Últimos 90 días',
};
...
export const KPICards: React.FC<KPICardsProps> = ({ kpis, loading = false }) => {
  const placeholders: MetricCardData[] = Array.from({ length: 4 }).map((_, index) => ({
      ...
      : kpis.map((kpi) => {
          ...
          const subtitle =
            typeof kpi.target === 'number'
              ? `Objetivo ${FunnelsAdquisicionService.formatKpiValue({
                  ...kpi,
                  value: kpi.target,
                })}`

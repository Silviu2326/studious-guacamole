...
import { ArrowUpRight, CalendarDays, Link, Users, Video } from 'lucide-react';
import { Badge, Button, Card } from '../../../components/componentsreutilizables';
import { ds } from '../../adherencia/ui/ds';
import { AcquisitionEvent } from '../types';
...
const typeIcon: Record<AcquisitionEvent['type'], React.ReactNode> = {
...
const statusBadge: Record<AcquisitionEvent['status'], { label: string; variant: 'blue' | 'yellow' | 'gray' }> = {
...
export const UpcomingEvents: React.FC<UpcomingEventsProps> = ({
  ...
  const placeholders = Array.from({ length: 3 }).map((_, index) => (
    <div key={`acquisition-event-skeleton-${index}`} className={`${ds.shimmer} h-24`} />
  ));
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className={`${ds.typography.bodySmall} ${ds.color.textSecondary} ${ds.color.textSecondaryDark}`}>
            Potencia adquisición, nurturing y cierre con experiencias en vivo accionables.
          </p>
        </div>
      </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    {event.funnelLink ? (
                      <Button
                        ...
                        variant="ghost"
                        size="sm"
                        className="inline-flex items-center gap-2 text-indigo-600 dark:text-indigo-300"
                      >
                        <a href={event.funnelLink}>

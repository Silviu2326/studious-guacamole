...
export const PreferenceCategoryRow: React.FC<PreferenceCategoryRowProps> = ({
  ...
  return (
    <Card padding="none" className="p-6 bg-white shadow-sm">
      {/* Header with Toggle */}
      ...
      <div className="flex gap-4">
        {availableChannels.map((channel) => (
          ...
          >
            {channel === 'email' ? (
              <Mail className={`w-6 h-6 ${selectedChannel === channel ? 'text-blue-600' : 'text-gray-400'}`} />
            ) : (
              <MessageCircle className={`w-6 h-6 ${selectedChannel === channel ? 'text-blue-600' : 'text-gray-400'}`} />
            )}
            <span className={`text-sm font-medium ${
              isSubscribed && selectedChannel === channel ? 'text-blue-600' : 'text-gray-600'
            }`}>

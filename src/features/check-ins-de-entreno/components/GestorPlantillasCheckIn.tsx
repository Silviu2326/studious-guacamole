...
interface GestorPlantillasCheckInProps {
  isOpen: boolean;
  onClose: () => void;
  clienteId?: string;
}

export const GestorPlantillasCheckIn: React.FC<GestorPlantillasCheckInProps> = ({
  isOpen,
  onClose,
  clienteId,
}) => {
  const [plantillas, setPlantillas] = useState<PlantillaCheckIn[]>([]);
  const [activaId, setActivaId] = useState<string | null>(null);
  const [showForm, setShowForm] = useState(false);
  const [editing, setEditing] = useState<PlantillaCheckIn | null>(null);
  ...
  const handleEdit = (tpl: PlantillaCheckIn) => {
    setEditing(tpl);
    setForm({
      nombre: tpl.nombre,
  ...
  const handleDelete = async (tpl: PlantillaCheckIn) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Plantillas de Check-in">

...
export const FacturacionManager: React.FC<FacturacionManagerProps> = ({ initialTab = 'dashboard', onTabChange }) => {
      ...
      <Card className="p-0 bg-white shadow-sm">
        <div className="px-4 py-3">
          ...
          >
            {tabs
              .filter(tab => {
                // If we're in the "facturacion" context (calendario, facturas, cobros, facturas-vencidas, ingresos-servicio)
                // only show those tabs. Otherwise show all tabs.
                const facturacionTabs = ['calendario', 'facturas', 'cobros', 'facturas-vencidas', 'ingresos-servicio'];
                if (facturacionTabs.includes(initialTab)) {
                  return facturacionTabs.includes(tab.id);
                }
                return true;
              })
              .map(({ id, label, icon }) => {

...
export const GoalTracker: React.FC<GoalTrackerProps> = ({ role, compact = false, category, onError }) => {
  ...
  const getStatusBadge = (status: Objective['status']) => {
    const statusConfig: Record<string, { label: string; variant: 'blue' | 'purple' | 'green' | 'yellow' | 'red' }> = {
      ...
      on_track: { label: 'En camino', variant: 'blue' as const },
      achieved: { label: 'Alcanzado', variant: 'green' as const },
      completed: { label: 'Completado', variant: 'green' as const },
      at_risk: { label: 'En riesgo', variant: 'yellow' as const },
      failed: { label: 'Fallido', variant: 'red' as const },
    };

    const config = statusConfig[status] || { label: 'Desconocido', variant: 'blue' as const };
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 bg-gray-100 rounded-lg p-1">
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${viewMode === 'cards' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-900'
                }`}
            >
              Tarjetas
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${viewMode === 'timeline' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-900'
                }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${viewMode === 'analysis' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:text-gray-900'
                }`}
            >
              ...
              <Card key={objective.id} className="p-6 bg-white shadow-sm border-l-4 border-l-blue-500">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        {projection.daysRemaining > 0 && (
                          <span className={`text-xs font-medium px-2 py-0.5 rounded ${projection.onTrack ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                            }`}>
                    ...
                    {allMilestones.map((milestone, index) => {
                      ...
                      return (
                        <div key={milestone.id} className="flex items-start gap-4 relative">
                          ...
                          <div className={`relative z-10 flex items-center justify-center w-8 h-8 rounded-full border-2 transition-all ${milestone.type === 'start'
                            ...
                            {milestone.type === 'milestone' && (
                              <div className="mt-2">
                                <div className="w-full bg-gray-100 rounded-full h-1.5">
                                  <div
                                    className={`h-1.5 rounded-full transition-all ${isAchieved ? 'bg-green-500' : 'bg-gray-300'
                                      }`}
                                    style={{
                                      width: `${Math.min(
                ...
                <div className="mt-6 pt-6 border-t border-gray-200">
                  ...
                  <div className="w-full bg-slate-100 rounded-full h-4 overflow-hidden">
                    <div
                      className={`h-4 rounded-full transition-all duration-500 ${objective.progress >= 100 ? 'bg-green-600' :
                  ...
                  <div className="mt-3 flex items-center justify-between text-xs">
                    ...
                    <span className={`font-medium flex items-center gap-1 ${projection.onTrack ? 'text-green-600' : 'text-yellow-600'
                      }`}>
                      {projection.onTrack ? (
                        <>
                  ...
                  return (
                    <div key={status} className="space-y-2">
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                        <div
                          className={`h-3 rounded-full transition-all ${status === 'achieved' ? 'bg-green-600' :
                      ...
                      <div key={objective.id} className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="w-full bg-gray-200 rounded-full h-2.5">
                            <div
                              className={`h-2.5 rounded-full transition-all ${projection.onTrack ? 'bg-green-600' : 'bg-yellow-600'
                                }`}
                              style={{ width: `${Math.min(objective.progress, 100)}%` }}
                            />
                          </div>
                          ...
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className={`font-semibold ${projection.onTrack ? 'text-green-600' : 'text-yellow-600'
                              }`}>
                ...
                <div className="mt-4 space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="w-full bg-slate-100 rounded-full h-3 overflow-hidden">
                      <div
                        className={`h-3 rounded-full transition-all duration-300 ${objective.progress >= 100 ? 'bg-green-600' :
                            objective.progress >= 75 ? 'bg-blue-600' :
                              objective.progress >= 50 ? 'bg-yellow-600' :
                                'bg-red-600'
                          }`}
                      ...
                      <div className="relative w-16 h-16">
                        <svg className="transform -rotate-90 w-16 h-16">
                          ...
                          <circle
                            ...
                            className={`transition-all duration-300 ${objective.progress >= 100 ? 'text-green-600' :
                                objective.progress >= 75 ? 'text-blue-600' :
                                  objective.progress >= 50 ? 'text-yellow-600' :
                                    'text-red-600'
                              }`}

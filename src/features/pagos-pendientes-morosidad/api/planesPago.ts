...
export const planesPagoAPI = {
  ...
  async marcarCuotaComoPagada(
    ...
    if (plan.montoPendiente === 0 || cuotasPagadas === totalCuotas) {
      ...
      plan.estado = plan.estadoPlan as any;
    }
  ...
  async getPlanesPagoPorCliente(clienteId: string): Promise<PlanPago[]> {
    ...
    return mockPlanesPago
      ...
      .map(plan => ({
        ...
        fechaFin: plan.fechaFin ? new Date(plan.fechaFin) : undefined,
        fechaFinEstimada: plan.fechaFinEstimada ? new Date(plan.fechaFinEstimada) : plan.fechaFin ? new Date(plan.fechaFin) : undefined,
  ...
  async actualizarPlanPago(
    ...
    return {
      ...
      fechaFinEstimada: planActualizado.fechaFinEstimada ? new Date(planActualizado.fechaFinEstimada) : planActualizado.fechaFin ? new Date(planActualizado.fechaFin) : undefined,

...
export default function SuscripcionesCuotasRecurrentesPage() {
  ...
  // Card-based navigation layer
  const [currentView, setCurrentView] = useState<'menu' | 'suscripciones' | 'proyecciones' | 'analytics'>('menu');

  // Original tab navigation (kept for internal navigation within cards)
  ...
  // Card menu for top-level navigation
  const renderMenu = () => {
    const activas = suscripciones.filter(s => s.estado === 'activa').length;
    const ingresosRecurrentes = suscripciones
      .filter(s => s.estado === 'activa')
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {/* 1. Suscripciones Card */}
        <FeatureCard
          title="Suscripciones"
          description="Gestión de suscripciones activas, renovaciones, sesiones y pagos"
          icon={<CreditCard size={24} />}
          color="sky"
          onClick={() => {
            setCurrentView('suscripciones');
            setTabActiva('activas');
          }}
          stats={[
            { label: 'Activas', value: activas.toString() },
            { label: 'Ingresos', value: `${ingresosRecurrentes.toFixed(0)}€` }
          ]}
        />
        ...
        <FeatureCard
          title="Proyecciones"
          description="Proyecciones de ingresos recurrentes y análisis de retención"
          icon={<TrendingUp size={24} />}
          color="purple"
          onClick={() => {
            setCurrentView('proyecciones');
            setTabActiva('proyecciones');
          }}
          stats={[
            { label: 'Proyección', value: `${(ingresosRecurrentes * 12).toFixed(0)}€` },
            { label: 'Anual', value: '12 meses' }
          ]}
        />
        ...
        <FeatureCard
          title="Analytics"
          description="Métricas de compromiso, retención y análisis de suscripciones"
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50">
      ...
      <div className="mx-auto max-w-[1600px] px-4 sm:px-6 lg:px-6 py-8">
        {currentView === 'menu' ? (
          ...
          <div className="space-y-6">
            {/* Métricas - Show only in suscripciones view */}
            {currentView === 'suscripciones' && <MetricCards data={metricas} columns={4} />}

            {/* Suscripciones view: Show tab system with filtered tabs */}
            {currentView === 'suscripciones' && (
              <>
                <Card className="p-0 bg-white shadow-sm">
                  <div className="px-4 py-3">
                    {/* Desktop: mostrar tabs filtrados */}
                    <div
                      role="tablist"
                    ...
                    >
                      {tabs.filter(tab => ['activas', 'renovaciones', 'sesiones', 'pagos-fallidos'].includes(tab.id)).map((tab) => {
                        ...
                        return (
                          <button
                            ...
                            className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-medium transition-all whitespace-nowrap ${isActive
            ...
            {/* Proyecciones view: Show proyecciones content directly */}
            {currentView === 'proyecciones' && (
              <div className="space-y-6">
                {esEntrenador && (
                  <ProyeccionesIngresosRetencion
                    entrenadorId={user?.id}
            ...
            {/* Analytics view: Show analytics content directly */}

...
import { Carrito, DatosCheckout, MetodoPago, MetodoPagoDetallado, DatosCheckoutGuardados, CarritoItem, CodigoPromocional, DescuentoFidelidad, PlanPagoFraccionado, PagoFraccionadoSeleccionado, Pedido, ItemCarrito } from '../types';
...
export const CheckoutManager: React.FC<CheckoutManagerProps> = ({
  ...
  const [metodosPago, setMetodosPago] = useState<MetodoPagoDetallado[]>([]);
  ...
  const calcularCarrito = React.useMemo((): Carrito => {
    ...
    if (codigoPromocional) {
      if (codigoPromocional.tipoDescuento === 'porcentaje') {
              ...
              return (
                <div key={paso.id} className="flex items-center flex-1">
                  <div className="flex flex-col items-center flex-1">
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center font-medium transition ${index < pasoActualIndex || paso.id === 'resultado'
                  ...
                  {index < pasos.length - 1 && (
                    <div
                      className={`flex-1 h-1 mx-2 ${index < pasoActualIndex ? 'bg-green-600' : 'bg-gray-200'
                        }`}
                    />
                  )}
                </div>
              ...
              <div key={paso.id} className="flex items-center flex-1">
                <div className="flex flex-col items-center flex-1">
                  <button
                    ...
                    className={`w-10 h-10 rounded-full flex items-center justify-center font-medium transition ${index < pasoActualIndex
                          ...
                          <div className="grid grid-cols-1 gap-2">
                            {planesPagoFraccionado.map((plan) => {
                              ...
                              return (
                                <label
                                  key={plan.id}
                                  className={`flex items-center gap-3 p-3 border rounded-lg cursor-pointer transition-all ${datos.pagoFraccionado?.planId === plan.id
